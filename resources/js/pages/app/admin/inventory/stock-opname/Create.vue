...
function getBatches(produkId: number | null): Batch[] {
    if (!produkId) return []
    const p = produks.value.find(p => p.id === produkId)
    return p?.batches || []
}
...
function submit() {
    ...
    router.post('/app/admin/inventory/stock-opname', {
        ...form.value,
    ...
    <AppLayout>
        <div class="flex-1 space-y-6 p-6">
            <PageHeader
                ...
                description="Record a physical stock count and compare against system records."
                :breadcrumbs="[
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div class="grid gap-6 lg:grid-cols-3">
                            ...
                            <div class="space-y-4">
                                ...
                                >
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        ...
                                        <FormField label="Batch / Purchase Item" :name="`items.${index}.pembelian_item_id`">
                                            <select
                                            ...
                                            >
                                                ...
                                                <option
                                                    v-for="b in getBatches(item.produk_id)"
                                                    :key="b.id"
                                                    :value="b.id"
                                                >
                                                    {{ b.no_po || `Batch #${b.id}` }} — Sisa: {{ b.qty_sisa }} {{ b.kondisi ? `(${b.kondisi})` : '' }}
                                                </option>
                                            </select>
                                        </FormField>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-3 gap-3">
                                        ...
                                        <FormField label="Physical Count" :name="`items.${index}.stok_fisik`" required>
                                            <Input
                                                v-model.number="item.stok_fisik"
                                                type="number"
                                                min="0"
                    ...
                    <div class="space-y-6">
                        <Card class="p-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="pt-2 border-t text-xs text-muted-foreground space-y-1">
                                    <p>Saved as <strong>Draft</strong>.</p>
                                    <p>Review variances, then <strong>Post</strong> to apply stock corrections.</p>
                                </div>
                            </div>
                        ...
                        <!-- Live Summary -->
                        <Card v-if="items.length > 0" class="p-6">
                            <h3 class="font-semibold mb-3">Live Summary</h3>
                            <div class="space-y-2 text-sm">
                                ...
                                <div class="flex justify-between">
                                    <span class="text-muted-foreground">Surplus items</span>
                                    <span class="text-green-600 font-medium">
                                        {{ items.filter(i => selisih(i) > 0).length }}
                                    </span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-muted-foreground">Deficit items</span>
                                    <span class="text-red-600 font-medium">
                                        {{ items.filter(i => selisih(i) < 0).length }}
                                    </span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-muted-foreground">Matched items</span>
